...
if(isset($_POST["action"]))
{
	...
	if($_POST['action'] == 'patient_register')
	{
		...
		else
		{
			...
			$mail->Port = '587';
			$mail->SMTPAuth = true;
			$mail->Username = 'dushyanthvj20010000@gmail.com';
			$mail->Password = 'Dushy.2001';
			$mail->SMTPSecure = 'tls';
			$mail->From = 'dushyanthv2001@gmail.com';
			$mail->FromName = 'GovernmentOfKarnataka';
	...
	if($_POST['action'] == 'fetch_schedule')
	{
		...
		$order_column = array('doctor_table.doctor_name', 'doctor_table.doctor_degree', 'doctor_table.doctor_expert_in', 
		'doctor_schedule_table.doctor_schedule_date', 'doctor_schedule_table.doctor_schedule_day', 'doctor_schedule_table.doctor_schedule_start_time');
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = $row["doctor_expert_in"].'</br><img src="images/'.$row["doctor_expert_in"].'.jpeg" width="120 " height="100"/>';
